...
- [Off The Wall](https://thurley.com/offthewall/) is another independent animation and games app. Comparing its public sender informed the fewer-replies policy; its application code is not included here.
...
Use the GitHub repository with Node 24 or later. No npm dependencies are required to run the site or tests.
...
Open `http://localhost:8080` in Chrome/Edge on the same computer. Web Bluetooth requires HTTPS or localhost. Production is https://raveboard.up.railway.app/ and uses the Dockerfile on Railway. 
