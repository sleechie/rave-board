import {buildPoints,frameFor} from './patterns.mjs?v=7';
import {loadHoldShapes,drawHoldShapes} from './hold-view.mjs?v=7';
import {quantize} from './protocol.mjs?v=3';
import { colorAt } from './effects.mjs?v=7';
...
let moving=!matchMedia('(prefers-reduced-motion: reduce)').matches, points=[], board=null,polygons=null,time=5, last=performance.now(), lastPaint=0, dirty=true;
...
try{const response=await fetch('/boards.json');if(!response.ok)throw Error();const data=await response.json();board=data.boards.find(b=>b.id===10);points=buildPoints(board,[1,20]);loadHoldShapes(board).then(shapes=>{polygons=shapes;dirty=true;}).catch(()=>{});}catch{toggle.disabled=true;toggle.textContent='Preview unavailable';}
function draw(now){
  ...
  if(points.length&&!document.hidden&&(dirty||moving)&&now-lastPaint>40){
    ...
    const frame=frameFor(points,'vortex','adapted',time,1),radius=Math.max(2,width/80);
    const colors=new Map(frame.map(p=>[p.position,quantize(p.rgb,3)]));
    const drawn=drawHoldShapes(context,{board,points,polygons,colors,width,height,ox,oy});
    if(!drawn)frame.forEach((led,i)=>{const p=points[i],color=`rgb(${led.rgb.join(',')})`;context.fillStyle=color;context.shadowColor=color;context.shadowBlur=radius*3;context.beginPath();context.arc(ox+p.u*width,oy+p.v*height,radius,0,Math.PI*2);context.fill();});context.shadowBlur=0;
  }
