import { EFFECTS,TOUR,mapPoints,makeFrame as originalFrame,hsv,gravityPhase } from './effects.mjs?v=7';
import { climberColor,climberFrame } from './climber.mjs?v=7';
import { letterPixel,cleanMessage,textDuration } from './lettering.mjs?v=6';
...
export const PATTERNS=[
  ...EFFECTS.map(effect=>({id:effect.id,name:effect.name,note:NOTES[effect.id]||effect.note})),
...
function adaptedColor(p,pattern,time,brightness,options) {
  if(pattern==='climber')return climberColor(p,time,brightness,true);
...
export function frameFor(points,pattern,variant,time,brightness=1,options={}) {
  if(!PATTERNS.some(p=>p.id===pattern))throw Error('Unknown pattern');
